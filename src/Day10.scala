...
object Day10 {
  ...
  def round(num:Int, lengths:List[Byte]):Unit = {
  ...
  def knotHash(input:String):String = {
    skip = 0
    curPos = 0
    list = Range(0, max).toList
    val bytes:List[Byte] = input.getBytes.toList ++ suffix
    //println(bytes)
    for (i <- 1 to 64) round(i, bytes)

    val result:List[Int] = (for (i <- 0 to 15) yield xor(list.slice(i * 16, i * 16 + 16))).toList

    //println(result + " length " + result.size)

    val hex = result.map { h =>
      var doubleHex = h.toHexString
      if (doubleHex.length < 2) doubleHex = "0" + doubleHex
      doubleHex
    }.mkString("")

    //println(hex + " " + hex.length)
